...
def key_callback(window, key, scancode, action, mods,id=None):
    if key == glfw.KEY_RIGHT and action == glfw.PRESS:
        # Apply a force to the ball when the spacebar is pressed
        ball_id = sim.model.body_name2id(id)
...
glfw.set_key_callback(viewer.window, lambda window, key, scancode, action, mods: key_callback(window, key, scancode, action, mods, id='ball'))
